        ...
        {news.map((itme, index) => (
          <SwiperSlide key={index}>
            <div className="flex flex-col sm:flex-row sm:justify-between items-center gap-12">
              ...
              <div className="py-4">
                <Link to="/">
                  <h3 className="text-lg font-medium hover:text-blue-500">{itme.title}</h3>
                </Link>
                <div className="w-12 h-[4px] bg-amber-500 mb-5 mt-2"></div>
                <p className="text-sm text-gray-600">{itme.description}</p>
              </div>
              

                {/* image */}
                <div className="flex-shrink-0 ">
                    <img src={itme.image} alt="Image"  className="w-full object-cover "/>
                </div>
